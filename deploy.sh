...
echo "Uploading css"
s3cmd -c s3cfg --exclude '*' --include '*.css'  --mime-type="text/css" --add-header='Content-Encoding: gzip' --add-header='Cache-Control: max-age=259200'  sync $OUTPUTDIR/* s3://www.chrisstucchio.com/
echo "Uploading javascript"
s3cmd -c s3cfg --exclude '*' --include '*.js'   --mime-type="text/javascript" --add-header='Content-Encoding: gzip' --add-header='Cache-Control: max-age=259200' sync $OUTPUTDIR/* s3://www.chrisstucchio.com/
echo "Uploading everything else"
s3cmd -c s3cfg --exclude '*.html' --exclude '*.css' --exclude '*.js' sync $OUTPUTDIR/ s3://www.chrisstucchio.com/
